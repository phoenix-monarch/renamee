...
async def rename_start(client, message):
    async def run():
        ...
        if file.file_size > 2000 * 1024 * 1024:
            await message.reply_text("Sorry, this bot does not support uploading files bigger than 2GB.")
            return
...
async def doc(bot, update):
    ...
    if c_caption:
        try:
            ...
            return await ms.edit(text=f"Your caption error exception keyword argument ●> ({e})")
    else:
