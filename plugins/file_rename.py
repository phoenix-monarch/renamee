...
async def rename_start(client, message):
    try:
        ...
        if file.file_size > 3500 * 1024 * 1024:
            await message.reply_text("Sᴏʀʀʏ Bʀᴏ Tʜɪꜱ Bᴏᴛ Iꜱ Dᴏᴇꜱɴ'ᴛ Sᴜᴩᴩᴏʀᴛ Uᴩʟᴏᴀᴅɪɴɢ Fɪʟᴇꜱ Bɪɢɢᴇʀ Tʜᴀɴ 3.5Gʙ")
        else:
...
async def doc(bot, update):
    ...
    try:
        ...
        return await ms.edit_text(str(e))
    duration = 0
    try:
    ...
    if c_caption:
        try:
            ...
            return await ms.edit_text(f"Yᴏᴜʀ Cᴀᴩᴛɪᴏɴ Eʀʀᴏʀ Exᴄᴇᴩᴛ Kᴇyᴡᴏʀᴅ Aʀɢᴜᴍᴇɴᴛ ●> ({e})")
    else:
    ...
    if value < media.file_size:
        await ms.edit_text("Tʀyɪɴɢ Tᴏ Uᴩʟᴏᴀᴅɪɴɢ....")
        type = update.data.split("_")[1]
        try:
            if type == "document":
                file = await bot.Client2.send_document(
                    Config.LOG_CHANNEL,
                    document=file_path,
                    thumb=ph_path,
                    caption=caption,
            ...
            if type == "video":
                file = await bot.Client2.send_video(
                    Config.LOG_CHANNEL,
                    document=file_path,
                    thumb=ph_path,
                    caption=caption,
            ...
            if type == "audio":
                file = await bot.Client2.send_audio(
                    Config.LOG_CHANNEL,
                    document=file_path,
                    thumb=ph_path,
                    caption=caption,
            ...
            if ph_path:
                os.remove(ph_path)
            return await ms.edit_text(f"Error {e}")
    await ms.delete()
    os.remove(file_path)
    if ph_path:
        os.remove(ph_path)
